...
const STORAGE_KEY = "medcore-maintenance-dismissed";
...
export function MaintenanceBanner() {
  ...
  useEffect(() => {
    ...
    if (val !== "true") {
      // Small delay so the page can render first — avoids janky mount
  ...
  // Don't show on login/register pages (auth flows)
      ...
      <div className="relative w-full max-w-lg overflow-hidden rounded-3xl border border-border bg-card shadow-2xl">
        ...
        <div className="px-8 pb-8 pt-10 text-center sm:px-10">
          ...
          <div className="relative mx-auto flex h-24 w-24 items-center justify-center">
            ...
            <span
              className="absolute inset-2 animate-pulse rounded-full bg-destructive/10"
              style={{ animationDuration: "2s" }}
            />

            {/* Icon circle */}
            <div className="relative flex h-20 w-20 items-center justify-center rounded-full bg-destructive text-destructive-foreground shadow-lg">
          ...
          <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-muted-foreground">
            ...
            of MedCore HMS. All screens and data are sample content to
          ...
          <p className="mt-5 text-[11px] text-muted-foreground">
            This notice will not appear again in this session.
          </p>
        </div>
      </div>
    </div>
